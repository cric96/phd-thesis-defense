...
SwarMDP\footnote{\fullcite{vsovsic2016inverse}} is a formal model composed of a \emph{swarming agent} $\mathbb{A}$ and an \emph{environment} $\mathbb{E}$:
  \begin{itemize}
...
\begin{block}{Learning in SwarMDP}
  \begin{itemize}
    ...
    \item Algorithms:
      \begin{itemize}
        \item \underline{Mean-field RL}\footnote[frame]{\fullcite{yang2018mean}}: Learning by considering neighboring agents as an approximation of the global state.
        \item \underline{Swarm RL}\footnote[frame]{\fullcite{iima2008swarm}}: A way to learn the Q-function in a swarm of agents.
      \end{itemize}
  \end{itemize}
\end{block}
...
\begin{frame}{Hybrid Aggregate Computing~\footnote[frame]{\fullcite{DBLP:conf/icdcs/AguzziCV22}}}
  ...
  \item \textbf{Execution Policies for Efficiency:}
      \begin{itemize}
      \item \textbf{Adaptive Collective Schedulers\footnote{\fullcite{DBLP:conf/acsos/AguzziCV22}:}} Shared-experience Q-learning to dynamically optimize execution for convergence speed or resource consumption.
  ...
  \item \textbf{\underline{Many-Agent RL through AC}:}
      \begin{itemize}
      \item \textbf{Field-Informed RL\footnote{\fullcite{acgnn}}:} Aggregate information (fields) guides policy learning in complex CPSW environments.
...
\begin{frame}{Engineering Methodologies II}
  ...
  \begin{itemize}
  \item \textbf{ScaRLib~\footnote{\fullcite{domini2023scarlib}}:} A library for cooperative many-agent reinforcement learning.
